...
function DeleteModal({
    name, count = 1, onCancel, onConfirm, deleting,
}: { name: string; count?: number; onCancel: () => void; onConfirm: () => void; deleting: boolean }) {
        ...
        >
            ...
            >
                ...
                <h3 style={{ color: '#1a3a6b', fontSize: 20, fontWeight: 700, marginBottom: 8 }}>
                    {count > 1 ? `${t('delete') || 'Delete'} ${count} ${t('analyses') || 'analyses'}` : t('deleteAnalysis') || 'Delete Analysis'}
                </h3>
...
function AnalysisRow({
    ...
    return (
        <div style={{
            background: isSelected ? '#eff6ff' : 'white', borderRadius: 14,
            boxShadow: '0 2px 10px rgba(26,63,95,0.06)',
            border: `1px solid ${isSelected ? '#93c5fd' : '#e8f0fa'}`,
            marginBottom: 12,
            overflow: 'hidden',
        ...
        >
            ...
            <div style={{
                ...
                <input type="checkbox" checked={isSelected} onChange={() => onToggle(record._id)} style={{ width: 16, height: 16, cursor: 'pointer', accentColor: '#1a3a6b' }} />
                {/* File icon */}
                <div style={{
...
export default function AudioHistoryPage() {
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            ...
            await Promise.all(ids.map(i => analysisApi.deleteAnalysis(i)));
    ...
    const handleBulkDelete = () => {
        setDeleteTarget({
            ...
            name: `${selected.size} analyses selected`,
            count: selected.size
        });
    };
            ...
            {loading ? (
                ...
                filtered.map(record => (
                    <AnalysisRow
                        key={record._id}
                        record={record}
                        isSelected={selected.has(record._id)}
                        onToggle={toggleOne}
